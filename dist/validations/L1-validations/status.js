...
function order_id_check(payload, externalData = {}) {
    const scope = payload_utils_1.default.getJsonPath(payload, "$");
    for (const testObj of scope) {
        testObj._EXTERNAL = externalData;
        const order_id = payload_utils_1.default.getJsonPath(testObj, "$.message.order_id");
        const nulls = ["null"];
        const skipCheck = false;
        if (skipCheck)
            continue;
        const output = validation_utils_1.default.NONE_IN(order_id, nulls);
        if (!output)
            return {
                valid: false,
                errorCode: 30000,
...
function validateStatus(payload, externalData = {}) {
