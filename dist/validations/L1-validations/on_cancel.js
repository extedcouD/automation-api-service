...
exports.validateOn_cancel = validateOn_cancel;
...
function validate_attribute_28(payload, externalData = {}) {
    ...
    for (const testObj of scope) {
        testObj._EXTERNAL = externalData;
        const attr = payload_utils_1.default.getJsonPath(testObj, "$.message.order.provider.descriptor.images[*].url");
        const nulls = ["null"];
        const skipCheck = false;
        if (skipCheck)
            continue;
...
function validate_attribute_44(payload, externalData = {}) {
    ...
    for (const testObj of scope) {
        testObj._EXTERNAL = externalData;
        const attr = payload_utils_1.default.getJsonPath(testObj, "$.message.order.cancellation.cancelled_by");
        const nulls = ["null"];
        const skipCheck = false;
        if (skipCheck)
            continue;
...
function validate_attribute_45(payload, externalData = {}) {
    ...
    for (const testObj of scope) {
        testObj._EXTERNAL = externalData;
        const attr = payload_utils_1.default.getJsonPath(testObj, "$.message.order.cancellation.time");
        const nulls = ["null"];
        const skipCheck = false;
        if (skipCheck)
            continue;
...
function validateOn_cancel(payload, externalData = {}) {
