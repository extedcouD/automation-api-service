...
function on_search(input) {
    ...
    for (const testObj of scope) {
        ...
        function validate_name_13(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_type_17(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_code_20(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_start_21(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_end_22(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_collected_by_23(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const attr = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].payments[*].collected_by");
                const validate = validation_utils_1.default.arePresent(attr);
                if (!validate) {
                    return [
                        {
	...
	> - **condition B**: $.message.catalog.providers[*].categories[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_enum_6(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].stops[*].type must **not** be present in the payload`,
        ...
        function validate_enum_11(input) {
            ...
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const enumList = ["QR"];
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].fulfillments[*].stops[*].authorization.type");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].stops[*].authorization.type must **not** be present in the payload`,
        ...
        function validate_enum_12(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].fulfillments[*].stops[*].authorization.status");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].stops[*].authorization.status must **not** be present in the payload`,
        ...
        function validate_enum_13(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].fulfillments[*].state.descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].state.descriptor.code must **not** be present in the payload`,
        ...
        function validate_enum_14(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].payments[*].status");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.catalog.providers[*].payments[*].collected_by must **not** be present in the payload`,
        ...
        function validate_enum_16(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].payments[*].type");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.catalog.providers[*].payments[*].type must **not** be present in the payload`,
        ...
        function validate_tag_0(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].items[*].tags[*].descriptor.code");
	...
	> - **condition B**: $.message.catalog.providers[*].items[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_0_FARE_POLICY(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].items[*].tags[?(@.descriptor.code=='FARE_POLICY')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_tag_1(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].fulfillments[*].state.descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(tagPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(tagPath, validTags);
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].state.descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_1_ROUTE_INFO(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].fulfillments[*].tags[?(@.state.descriptor.code=='ROUTE_INFO')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
        ...
        function validate_tag_1_TICKET_INFO(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].fulfillments[*].tags[?(@.state.descriptor.code=='TICKET_INFO')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
        ...
        function validate_tag_1_TRIP_DETAILS(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].fulfillments[*].tags[?(@.state.descriptor.code=='TRIP_DETAILS')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
        ...
        function validate_tag_1_INFO(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].fulfillments[*].tags[?(@.state.descriptor.code=='INFO')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
        ...
        function validate_tag_2(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].payments[*].tags[*].descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(tagPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(tagPath, validTags);
	...
	> - **condition B**: $.message.catalog.providers[*].payments[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_2_BUYER_FINDER_FEES(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].payments[*].tags[?(@.descriptor.code=='BUYER_FINDER_FEES')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
                const validValues = [
        ...
        function validate_tag_2_SETTLEMENT_TERMS(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].payments[*].tags[?(@.descriptor.code=='SETTLEMENT_TERMS')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
                const validValues = [
        ...
        function validate_tag_3(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.providers[*].descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(tagPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(tagPath, validTags);
	...
	> - **condition B**: $.message.catalog.providers[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_3_SCHEDULED_INFO(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.providers[*].tags[?(@.descriptor.code=='SCHEDULED_INFO')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
        ...
        function validate_tag_4(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.catalog.tags[*].descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(tagPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(tagPath, validTags);
	...
	> - **condition B**: $.message.catalog.tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_4_PAGINATION(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.catalog.tags[?(@.descriptor.code=='PAGINATION')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
                const validValues = [
