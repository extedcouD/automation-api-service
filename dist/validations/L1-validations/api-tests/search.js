...
function search(input) {
    ...
    for (const testObj of scope) {
        ...
        function validate_category_11(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
	...
	> - **condition B**: $.message.intent.fulfillment.vehicle.category must **not** be present in the payload`,
        ...
        function validate_enum_7(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.intent.fulfillment.type");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.intent.fulfillment.stops[*].type must **not** be present in the payload`,
        ...
        function validate_enum_9(input) {
            ...
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const enumList = ["QR"];
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.intent.fulfillment.stops[*].authorization.type");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.intent.fulfillment.stops[*].authorization.type must **not** be present in the payload`,
        ...
        function validate_enum_10(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.intent.fulfillment.stops[*].authorization.status");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.intent.fulfillment.stops[*].authorization.status must **not** be present in the payload`,
        ...
        function validate_enum_11(input) {
            ...
            for (const testObj of scope) {
                ...
                const enumPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.intent.fulfillment.state.descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(enumPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(enumPath, enumList);
	...
	> - **condition B**: $.message.intent.fulfillment.state.descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_0(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.intent.payment.tags[*].descriptor.code");
	...
	> - **condition B**: $.message.intent.payment.tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_0_BUYER_FINDER_FEES(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.intent.payment.tags[?(@.descriptor.code=='BUYER_FINDER_FEES')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
        ...
        function validate_tag_0_SETTLEMENT_TERMS(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.intent.payment.tags[?(@.descriptor.code=='SETTLEMENT_TERMS')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
                const validValues = [
        ...
        function validate_tag_1(input) {
            ...
            for (const testObj of scope) {
                ...
                const tagPath = json_path_utils_1.default.getJsonPath(testObj, "$.message.intent.tags[*].descriptor.code");
                const skipCheck = !validation_utils_1.default.arePresent(tagPath);
                if (skipCheck)
                    continue;
                const validate = validation_utils_1.default.allIn(tagPath, validTags);
	...
	> - **condition B**: $.message.intent.tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function validate_tag_1_PAGINATION(input) {
            const scope = json_path_utils_1.default.getJsonPath(input.payload, "$.message.intent.tags[?(@.descriptor.code=='PAGINATION')]");
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const subTags = json_path_utils_1.default.getJsonPath(testObj, "$.list[*].descriptor.code");
