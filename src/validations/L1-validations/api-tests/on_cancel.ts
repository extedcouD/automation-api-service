...
export default function on_cancel(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function Attri_Required_MESSAGE_ORDER_STATUS(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_MESSAGE_ORDER_STATUS**: $.message.order.status must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_ORDER_ITEMS_CATEGORY_IDS(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ORDER_ITEMS_CATEGORY_IDS**: $.message.order.items[*].category_ids[*] must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_DESCRIPTOR_IMAGES_URL(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_DESCRIPTOR_IMAGES_URL**: $.message.order.provider.descriptor.images[*].url must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_TIME_RANGE_START**: $.message.order.provider.time.range.start must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_TIME_RANGE_END**: $.message.order.provider.time.range.end must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_PAYMENTS_PARAMS_BANK_CODE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_PAYMENTS_PARAMS_BANK_CODE**: $.message.order.payments[*].params.bank_code must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_PAYMENTS_PARAMS_BANK_ACCOUNT_NUMBER(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_PAYMENTS_PARAMS_BANK_ACCOUNT_NUMBER**: $.message.order.payments[*].params.bank_account_number must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_URL(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_URL**: $.message.order.cancellation_terms[*].external_ref.url must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_MIMETYPE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_MIMETYPE**: $.message.order.cancellation_terms[*].external_ref.mimetype must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_ORDER_CANCELLATION_CANCELLED_BY(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ORDER_CANCELLATION_CANCELLED_BY**: $.message.order.cancellation.cancelled_by must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_ORDER_CANCELLATION_TIME(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ORDER_CANCELLATION_TIME**: $.message.order.cancellation.time must be present in the payload`,
                        },
                    ];
                }
  ...
  - **condition Enum_Required_QUOTE_BREAKUP_TITLE.2**: $.message.order.quote.breakup[*].title must be present in the payload`,
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition BUYER_FINDER_FEES_Tag_Required_TAGS_DESCRIPTOR_CODE**: every element of $.message.order.payments[*].tags[*].descriptor.code must be in ["BUYER_FINDER_FEES", "SETTLEMENT_TERMS"]
	...
	> - **condition B**: $.message.order.payments[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        const testFunctions: testFunctionArray = [
            ...
            Attri_Required_MESSAGE_ORDER_STATUS,
            ...
            Attri_Required_ORDER_ITEMS_CATEGORY_IDS,
            ...
            Attri_Required_DESCRIPTOR_IMAGES_URL,
            ...
            Attri_Required_PAYMENTS_PARAMS_BANK_CODE,
            Attri_Required_PAYMENTS_PARAMS_BANK_ACCOUNT_NUMBER,
            Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_URL,
            Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_MIMETYPE,
            Attri_Required_ORDER_CANCELLATION_CANCELLED_BY,
            Attri_Required_ORDER_CANCELLATION_TIME,
