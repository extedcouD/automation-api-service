...
export default function on_confirm(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function Attri_Required_ITEM_PRICE_VALUE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ITEM_PRICE_VALUE**: $.message.order.quote.breakup[*].item.price.value must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_ITEM_PRICE_CURRENCY(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ITEM_PRICE_CURRENCY**: $.message.order.quote.breakup[*].item.price.currency must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_QUANTITY_SELECTED_COUNT_2(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_QUANTITY_SELECTED_COUNT_2**: $.message.order.quote.breakup[*].item.quantity.selected.count must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_MESSAGE_ORDER_CREATED_AT(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_MESSAGE_ORDER_CREATED_AT**: $.message.order.created_at must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_MESSAGE_ORDER_UPDATED_AT(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_MESSAGE_ORDER_UPDATED_AT**: $.message.order.updated_at must be present in the payload`,
                        },
                    ];
                }
  ...
  - **condition Enum_Required_QUOTE_BREAKUP_TITLE.2**: $.message.order.quote.breakup[*].title must be present in the payload`,
	...
	> - **condition B**: $.message.order.payments[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        const testFunctions: testFunctionArray = [
            ...
            Attri_Required_ITEM_PRICE_VALUE,
            Attri_Required_ITEM_PRICE_CURRENCY,
            Attri_Required_QUANTITY_SELECTED_COUNT_2,
            Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_URL,
            Attri_Required_CANCELLATION_TERMS_EXTERNAL_REF_MIMETYPE,
            Attri_Required_MESSAGE_ORDER_CREATED_AT,
            Attri_Required_MESSAGE_ORDER_UPDATED_AT,
