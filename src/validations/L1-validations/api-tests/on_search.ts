...
export default function on_search(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function Attri_Required_CATALOG_PROVIDERS_ID(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_CATALOG_PROVIDERS_ID**: $.message.catalog.providers[*].id must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_PROVIDERS_DESCRIPTOR_NAME(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_PROVIDERS_DESCRIPTOR_NAME**: $.message.catalog.providers[*].descriptor.name must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_PROVIDERS_FULFILLMENTS_ID(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_PROVIDERS_FULFILLMENTS_ID**: $.message.catalog.providers[*].fulfillments[*].id must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_STOPS_LOCATION_GPS**: $.message.catalog.providers[*].fulfillments[*].stops[*].location.gps must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_PROVIDERS_CATEGORIES_ID(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_PROVIDERS_CATEGORIES_ID**: $.message.catalog.providers[*].categories[*].id must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_TIME_RANGE_START(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_TIME_RANGE_START**: $.message.catalog.providers[*].time.range.start must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_TIME_RANGE_END(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_TIME_RANGE_END**: $.message.catalog.providers[*].time.range.end must be present in the payload`,
                        },
                    ];
                }
  ...
  - **condition Enum_Required__CONTEXT_DOMAIN.2**: $.context.domain must be present in the payload`,
        ...
        function Enum_Required_CATEGORIES_DESCRIPTOR_CODE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_CATEGORIES_DESCRIPTOR_CODE**: all of the following sub conditions must be met:
  ...
  - **condition Enum_Required_CATEGORIES_DESCRIPTOR_CODE.2**: $.message.catalog.providers[*].categories[*].descriptor.code must be present in the payload`,
        ...
        function Enum_Required_PROVIDERS_DESCRIPTOR_CODE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_PROVIDERS_DESCRIPTOR_CODE**: every element of $.message.catalog.providers[*].descriptor.code must be in ["SJT", "SFSJT", "RJT", "PASS"]
  ...
  - **condition Enum_Required_FULFILLMENTS_VEHICLE_CATEGORY.2**: $.message.catalog.providers[*].fulfillments[*].vehicle.category must be present in the payload`,
        ...
        function Enum_Required_PROVIDERS_FULFILLMENTS_TYPE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_PROVIDERS_FULFILLMENTS_TYPE**: all of the following sub conditions must be met:
  ...
  - **condition Enum_Required_PROVIDERS_FULFILLMENTS_TYPE.2**: $.message.catalog.providers[*].fulfillments[*].type must be present in the payload`,
        ...
        function Enum_Required_FULFILLMENTS_STOP_TYPE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_FULFILLMENTS_STOP_TYPE**: every element of $.message.catalog.providers[*].fulfillments[*].stops[*].type must be in ["START", "END", "INTERMEDIATE_STOP", "TRANSIT_STOP"]
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_STOPS_AUTHORIZATION_TYPE**: every element of $.message.catalog.providers[*].fulfillments[*].stops[*].authorization.type must be in ["QR"]
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_STOPS_AUTHORIZATION_STATUS**: every element of $.message.catalog.providers[*].fulfillments[*].stops[*].authorization.status must be in ["UNCLAIMED", "CLAIMED"]
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].stops[*].authorization.status must **not** be present in the payload`,
        ...
        function Enum_Required_PROVIDERS_PAYMENTS_STATUS(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_PROVIDERS_PAYMENTS_STATUS**: every element of $.message.catalog.providers[*].payments[*].status must be in ["NOT-PAID", "PAID"]
	...
	> - **condition B**: $.message.catalog.providers[*].payments[*].status must **not** be present in the payload`,
        ...
        function Enum_Required_PROVIDERS_PAYMENTS_COLLECTED_BY(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_PROVIDERS_PAYMENTS_COLLECTED_BY**: all of the following sub conditions must be met:
  ...
  - **condition Enum_Required_PROVIDERS_PAYMENTS_COLLECTED_BY.2**: $.message.catalog.providers[*].payments[*].collected_by must be present in the payload`,
        ...
        function Enum_Required_PROVIDERS_PAYMENTS_TYPE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Enum_Required_PROVIDERS_PAYMENTS_TYPE**: every element of $.message.catalog.providers[*].payments[*].type must be in ["PRE-ORDER", "ON-FULFILLMENT", "POST-FULFILLMENT"]
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition FARE_POLICY_Tag_Required_ITEMS_DESCRIPTOR_CODE**: every element of $.message.catalog.providers[*].items[*].descriptor.code must be in ["FARE_POLICY"]
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition ROUTE_INFO_Tag_Required_STATE_DESCRIPTOR_CODE**: every element of $.message.catalog.providers[*].fulfillments[*].state.descriptor.code must be in ["ROUTE_INFO", "TICKET_INFO", "TRIP_DETAILS", "INFO"]
	...
	> - **condition B**: $.message.catalog.providers[*].fulfillments[*].state.descriptor.code must **not** be present in the payload`,
        ...
        function BUYER_FINDER_FEES_Tag_Required_TAGS_DESCRIPTOR_CODE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition BUYER_FINDER_FEES_Tag_Required_TAGS_DESCRIPTOR_CODE**: every element of $.message.catalog.providers[*].payments[*].tags[*].descriptor.code must be in ["BUYER_FINDER_FEES", "SETTLEMENT_TERMS"]
	...
	> - **condition B**: $.message.catalog.providers[*].payments[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function SCHEDULED_INFO_Tag_Required_PROVIDERS_DESCRIPTOR_CODE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition SCHEDULED_INFO_Tag_Required_PROVIDERS_DESCRIPTOR_CODE**: every element of $.message.catalog.providers[*].descriptor.code must be in ["SCHEDULED_INFO"]
	...
	> - **condition B**: $.message.catalog.providers[*].descriptor.code must **not** be present in the payload`,
        ...
        function PAGINATION_Tag_Required_TAGS_DESCRIPTOR_CODE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition PAGINATION_Tag_Required_TAGS_DESCRIPTOR_CODE**: every element of $.message.catalog.tags[*].descriptor.code must be in ["PAGINATION"]
	...
	> - **condition B**: $.message.catalog.tags[*].descriptor.code must **not** be present in the payload`,
        ...
        const testFunctions: testFunctionArray = [
            ...
            Attri_Required_CATALOG_PROVIDERS_ID,
            Attri_Required_PROVIDERS_DESCRIPTOR_NAME,
            Attri_Required_PROVIDERS_FULFILLMENTS_ID,
            Attri_Required_STOPS_LOCATION_GPS,
            Attri_Required_PROVIDERS_CATEGORIES_ID,
            Attri_Required_TIME_RANGE_START,
            Attri_Required_TIME_RANGE_END,
            Enum_Required__CONTEXT_ACTION,
            Enum_Required_LOCATION_COUNTRY_CODE,
            Enum_Required__CONTEXT_DOMAIN,
            Enum_Required_CATEGORIES_DESCRIPTOR_CODE,
            Enum_Required_PROVIDERS_DESCRIPTOR_CODE,
            Enum_Required_FULFILLMENTS_VEHICLE_CATEGORY,
            Enum_Required_PROVIDERS_FULFILLMENTS_TYPE,
            Enum_Required_FULFILLMENTS_STOP_TYPE,
            Enum_Required_STOPS_AUTHORIZATION_TYPE,
            Enum_Required_STOPS_AUTHORIZATION_STATUS,
            Enum_Required_PROVIDERS_PAYMENTS_STATUS,
            Enum_Required_PROVIDERS_PAYMENTS_COLLECTED_BY,
            Enum_Required_PROVIDERS_PAYMENTS_TYPE,
            ...
            validate_tag_1_INFO,
            BUYER_FINDER_FEES_Tag_Required_TAGS_DESCRIPTOR_CODE,
            validate_tag_2_BUYER_FINDER_FEES,
            validate_tag_2_SETTLEMENT_TERMS,
            SCHEDULED_INFO_Tag_Required_PROVIDERS_DESCRIPTOR_CODE,
            validate_tag_3_SCHEDULED_INFO,
            PAGINATION_Tag_Required_TAGS_DESCRIPTOR_CODE,
