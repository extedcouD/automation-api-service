...
export default function init(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function Attri_Required_ORDER_PAYMENTS_COLLECTED_BY(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ORDER_PAYMENTS_COLLECTED_BY**: $.message.order.payments[*].collected_by must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_ORDER_PAYMENTS_STATUS(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ORDER_PAYMENTS_STATUS**: $.message.order.payments[*].status must be present in the payload`,
                        },
                    ];
                }
        ...
        function Attri_Required_ORDER_PAYMENTS_TYPE(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Attri_Required_ORDER_PAYMENTS_TYPE**: $.message.order.payments[*].type must be present in the payload`,
                        },
                    ];
                }
  ...
  - **condition Enum_Required__CONTEXT_DOMAIN.2**: $.context.domain must be present in the payload`,
	...
	> - **condition B**: $.message.order.payments[*].tags[*].descriptor.code must **not** be present in the payload`,
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_tag_0_BUYER_FINDER_FEES**: every element of $.message.order.payments[*].tags[?(@.descriptor.code=='BUYER_FINDER_FEES')].list[*].descriptor.code must be in ["BUYER_FINDER_FEES_PERCENTAGE", "BUYER_FINDER_FEES_AMOUNT"]`,
                        },
                    ];
                }
        ...
        const testFunctions: testFunctionArray = [
            ...
            Attri_Required_ORDER_PAYMENTS_COLLECTED_BY,
            Attri_Required_ORDER_PAYMENTS_STATUS,
            Attri_Required_ORDER_PAYMENTS_TYPE,
